# How TPM Was Accidentally Maxed Out — And How It Was Fixed
...
2. **Session state goes to the next agent** — Storing large blobs in `session.state` means they end up in the next agent's prompt. Prefer references (filenames, cache keys) over raw data.

3. **Artifacts ≠ context** — ADK's `save_artifact` stores binaries separately. They do not bloat the LLM unless explicitly passed. Use artifacts for generated files; use status strings for tool returns.
...
**DISCLAIMER** — Spotting the issues, the design of the solution, the core code, and testing were done by the author. The majority of commodity code was produced by AI or pair programming and has been fully verified or modified by the author.
