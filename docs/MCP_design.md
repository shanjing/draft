...
- **Read-only** — no write operations that change repo registration or config (e.g. no `add_source`). Fits future use as a knowledge source for an SRE admin agent (runbooks, proprietary tech docs).
- **Low latency** — fast tools return quickly; streaming and long-running ops are explicitly tiered so clients get immediate feedback.
- **Lightweight** — minimal dependencies, simple process, low resource footprint; easy to run alongside the UI or in a small container.
- **Optional security** — Bearer token auth; optional SSL/TLS for HTTP when deployed for SRE/production (e.g. behind reverse proxy or TLS termination). Stdio remains for local, trusted use.
- **Auditable** — every request and response is traceable (e.g. request IDs, structured logs) so tool calls can be audited and debugged.
- **Observable** — performance and reliability are measurable (e.g. latency percentiles such as p99, uptime, error rates) so human SREs or SRE Agents can monitor and alert.
...
Request ID is set in a context var at request start (HTTP: from `Mcp-Session-Id` header in middleware; stdio: generated UUID). See [observability_design.md](observability_design.md) for full propagation and instrumentation.
...
MCP observability uses **OpenTelemetry** (OTel) and **structured logs** to answer pipeline-level and audit questions. Design principle: **Prometheus/Grafana** = "Is the system healthy?"; **OTel traces** = "Where did this request go wrong and why?"; **Structured logs** = "What exactly happened in this request?"

- **Tool boundary:** One OTel span per tool call (`mcp.tool.<name>`), with `request_id` and `transport`. Request ID is set in a context var at request start (HTTP: `Mcp-Session-Id`; stdio: generated UUID).
- **Structured logging:** One JSON log line per tool call (ts, request_id, tool, transport, duration_ms, status, error_type) when `--log-json` or `MCP_LOG_JSON=1`. Emitted from the same dispatcher that starts the span.
- **query_docs:** When this tool runs, RAG child spans (retrieval, rerank, LLM generation) attach under the tool span via OTel context propagation; see project-level RAG observability in the main doc.
- **Source of truth:** Logs = audit trail; OTel spans = latency and error analysis. Optional `/metrics` for Prometheus scrape when OTel is not used for metrics.

**Full design (request_id propagation, log schema, optional /metrics, relationship to RAG):** [observability_design.md](observability_design.md) is the source of truth.
...
- Note: will use the agentic RAG router once implemented (see [agentic_RAG_design.md](agentic_RAG_design.md))
